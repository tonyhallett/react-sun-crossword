    ...
    var parseResults = function parseResults(results, confidences) {
        ...
        for (var i = 0; i < results.length; i++) {
            ...
            for (var j = 0, l = commandsList.length; j < l; j++) {
                ...
                console.log("Testing regex: " + currentCommand.command.toString());
                console.log("against: " + commandText + "!");
                ...
                if (result) {
                    ...
                    // execute the matched command - new args confidences[i] and originalPhrase
                    var args = [currentCommand.originalPhrase, confidences[i]].concat(parameters);
                    currentCommand.callback.apply(this, args);
    ...
    annyang = {
        ...
        init: function init(commands) {
            ...
            */
            ...
            if (commands) {
                if (commands.length) {
                    commands.forEach(function (cmds) { return _this.addCommands(cmds); });
                }
                else {
                    this.addCommands(commands);
                }
