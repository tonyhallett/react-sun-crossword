...
import { CrosswordModel, Square, IWord } from '../src/models/index'
...
function getModelWithSelectedSquare(rowIndex: number, colIndex: number) {
    var crosswordModel = getModel();
    var selectedSquare = crosswordModel.grid[rowIndex][colIndex];
    selectedSquare.selected = true;
    crosswordModel.selectedSquare = selectedSquare;
    return crosswordModel;
}
function getModelWithSelectedSquareWithGuess(rowIndex: number, colIndex: number, guess: string) {
    var model = getModelWithSelectedSquare(rowIndex, colIndex);
    model.selectedSquare.guess = guess;
    return model;
}
describe('Crossword puzzle input', () => {
    describe('alhpa keys,lower or upper', () => {
        describe('should set the guess on the selected square with uppercase of key', () => {
            pit('a', 'a', 'A');
            pit('A', 'A', 'A');
            //could be more thorough and demonstrate that this has not effected any of the others
            pits((keyValue, expectedGuess) => {
                var model = getModelWithSelectedSquare(0, 0);
                var selectedWord = model.selectedSquare.acrossWord;
                selectedWord.select();
                model.selectedWord = selectedWord;

                var wrapper = mount(<CrosswordPuzzle crosswordModel={model} />);
                var crosswordPuzzle = wrapper.instance() as CrosswordPuzzle;
                crosswordPuzzle.keyGuess(null, keyValue);
                wrapper.update();//necessary
                var crossword = wrapper.find(Crossword).first();
                var crosswordProps = crossword.prop('squares');
                expect(crosswordProps[0][0].guess).toBe(expectedGuess);
            });
        });
        describe('it should move forward the selected square in the selected word if not the last in the word, selected word should not change', () => {
            interface RowCol {
                row: number,
                col: number
            }
            ...
            pits((selectedRowCol: RowCol, acrossWord: boolean, expectedSelectedSquare: RowCol) => {
                ...
                var selectedSquare = model.selectedSquare;
                var selectedWord: IWord;
                if (acrossWord) {
                    selectedWord = selectedSquare.acrossWord;
                } else {
                    selectedWord = selectedSquare.downWord;
                }
                model.selectedWord = selectedWord;
                selectedWord.select();

                var selectedSquaresRowCol = selectedWord.squares.map((sq) => {
                    return { row: sq.rowIndex, col: sq.columnIndex };
                });
                var wrapper = mount(<CrosswordPuzzle crosswordModel={model} />);
                var crosswordPuzzle = wrapper.instance() as CrosswordPuzzle;
                crosswordPuzzle.keyGuess(null,"A");
                wrapper.update();//necessary
                var crossword = wrapper.find(Crossword).first();
                var crosswordProps = crossword.prop('squares');

                expect(crosswordProps[expectedSelectedSquare.row][expectedSelectedSquare.col].isSelected).toBe(true);
                selectedSquaresRowCol.forEach(rowCol => {
                    expect(crosswordProps[rowCol.row][rowCol.col].isWordSelected).toBe(true);
                });
        });
    });
    
    describe('backspace', () => {
        it('should not throw if no square is selected', () => {
            var model = getModel();
            
            var wrapper = mount(<CrosswordPuzzle crosswordModel={model} />);
            var crosswordPuzzle = wrapper.instance() as CrosswordPuzzle;
            crosswordPuzzle.backspace();
            wrapper.update();//necessary
            var crossword = wrapper.find(Crossword).first();
            var crosswordProps = crossword.prop('squares');
            expect(crosswordProps[0][0].guess).toBe("");
        });
        it('should reset the guess on the selected square', () => {
            var model = getModelWithSelectedSquareWithGuess(0, 0, "A");
            var selectedWord = model.selectedSquare.acrossWord;
            selectedWord.select();
            model.selectedWord = selectedWord;
        ...
        describe('it should move back the selected square in the selected word if not the first in the word, selected word should not change', () => {
            interface RowCol {
                row: number,
                col:number
            }
            pit('not first across', { row: 0, col: 1 }, true, { row:0,col:0});
            pit('not first down', { row: 1, col: 0 }, false, { row: 0, col: 0 });
            pit('not first intersection across', { row: 2, col: 2 }, true, { row: 2, col: 1 })
            pit('not first intersection down', { row: 2, col: 2 }, false, { row: 1, col: 2 })
            pit('first across not on edge', { row: 0, col: 10 }, true, { row: 0, col: 10 })
            pit('first across on edge intersection', { row: 0, col: 0 }, true, { row: 0, col: 0 })
            pit('first down not on edge', { row: 5, col: 8 }, false, { row: 5, col: 8 })
            pit('first down on edge intersection', { row: 0, col: 0 }, false, { row: 0, col: 0 })
